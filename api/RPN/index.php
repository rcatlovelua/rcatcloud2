...
// ============================================
// ТОЧНАЯ БЛОКИРОВКА ТОЛЬКО FlClash
// ============================================
$blocked = false;

// FlClash всегда использует Dart/Flutter в User-Agent
// и содержит "clash" в имени пакета
$flClashSignatures = [
    'flclash',
    'FlClash',
];

// Проверяем ТОЛЬКО если User-Agent содержит "clash" И "dart"/"flutter"
if (
    stripos($userAgent, 'clash') !== false && 
    stripos($userAgent, 'flclash') !== false
) {
    $blocked = true;
}

// Дополнительно: проверяем заголовок X-Requested-With только для FlClash
if (
    isset($_SERVER['HTTP_X_REQUESTED_WITH']) && 
    stripos($_SERVER['HTTP_X_REQUESTED_WITH'], 'flclash') !== false
) {
    $blocked = true;
}

// Блокируем только если точно определили FlClash
if ($blocked) {
    http_response_code(403);
    header('Content-Type: application/json');
    echo json_encode(['error' => 'FlClash is not supported. Please use another client.']);
    exit;
}

// ============================================
// ОПРЕДЕЛЕНИЕ V2RAY/ПРОКСИ КЛИЕНТОВ
// ============================================
$isV2RayClient = false;

// Заголовки, которые отправляют ТОЛЬКО прокси-клиенты при запросе подписки
$proxyHeaders = [
    'HTTP_PROFILE_UPDATE_INTERVAL',
    'HTTP_SUBSCRIPTION_USERINFO',
    'HTTP_X_SUBSCRIPTION',
    'HTTP_X_V2RAY',
    'HTTP_USER_AGENT_CONFIG',
];
...
// Проверка User-Agent на известные прокси-клиенты
if (!$isV2RayClient && !empty($userAgent)) {
    $proxyClients = [
        'v2ray', 'v2rayNG', 'v2rayN', 'V2Box', 'V2RayU', 'V2RayX',
        'xray', 'XrayNG', 'sing-box', 'SFA', 'SFI',
        'streisand', 'foxray', 'hiddify', 'hiddifyNG',
        'matsuri', 'v2rayA', 'qv2ray', 'nekobox', 'nekoray',
        'shadowrocket', 'quantumult', 'surge', 'loon', 'stash', 'egern',
        'karing', 'hiddify-next', 'mahsang', 'napsternetv',
        'clash', 'clashx', 'clash-verge', 'clash.meta', 'mihomo',
        'flutter', 'dart',
    ];

    foreach ($proxyClients as $client) {
...
// Проверка наличия "subscription" или "profile" в URL
$requestUri = $_SERVER['REQUEST_URI'] ?? '';
if (
    stripos($requestUri, 'sub') !== false || 
    stripos($requestUri, 'profile') !== false ||
    stripos($requestUri, 'config') !== false
) {
    $isV2RayClient = true;
}
...
if ($isV2RayClient) {
    ...
    // Заголовок для информации о подписке (поддержка v2rayNG и подобных)
...
YAML;
    ...
    // Используем gzip сжатие если поддерживается
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .feature p { font-size: 13px; color: rgba(255,255,255,0.6); }
        .speed-badge {
...
<body>
    <div class="container">
        ...
        <h1>RPN VPN</h1>
        <p class="subtitle">
            Высокоскоростной VPN сервис с пропускной способностью до 1 Гбит/с
        </p>
        <div class="speed-badge">🚀 До 1 Гбит/с</div>
        <div class="features">
            <div class="feature">
        ...
        <p style="color: rgba(255,255,255,0.5); font-size: 14px; margin-top: 20px;">
            Поддерживаемые клиенты: v2rayNG, Sing-box, Streisand, FoxRay, Shadowrocket, Clash и другие
        </p>
        <div class="footer">
            © <?= date('Y') ?> RPN VPN Service. Все права в опасности.
        </div>
    </div>
</body>
</html>
